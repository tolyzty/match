...
<html>
  ...
  <body>
<h2 style="text-align: center;">新增角色权限

</h2>
<input type="hidden" name="rrid" value="${roleId}" id="rrid"/>
...
var zTree; //定义Ztree对象
  var setting = {
  		 view: {
		    ...
		    selectedMulti: false //设置是否允许同时选中多个节点
  			...
  			enable:true,
  			type:"post",
  			url:"<%=path%>/ht/role/ajaxMenu.do"
  		},data:{
  			simpleData: {
				enable:true,
				 idKey: "menuId",    //设置节点唯一标识属性名称  
  	 ...
  	 var rrid=$('#rrid').val();
  	 var url = '<%=path%>/ht/role/menuAdd.do';
  	 var urls = '<%=path%>/ht/role/menuAddk.do';
  ...
  $(document).ready(function(){  
	  $.ajax({
	        ...
	        url: "<%=path%>/ht/role/ajaxMenu.do",  //请求的URL路径  
	        beforeSend:function(){}, 
	  ...
	  /*    var t = $("#treeDemos");
	     ...
	     $.fn.zTree.init($("#treeDemos"), setting, treeNodes);
		 	...
		 	var newsobj = data.lists;
		 	 if(newsobj==""){
			    $("#menuad").on('click',function(){
			        ...
			        var zTrees = $.fn.zTree.getZTreeObj("treeDemos");//实例化
	 	          for(var i=0;i<newsobj.length;i++){
				     var node= zTrees.getNodeByParam("menuId",newsobj[i].menuId);//为节点赋值参数
